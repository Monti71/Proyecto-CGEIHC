...
int main()
{
	...
	Model casaBrujas("resources/objects/Brujas/CasaBrujas.obj");	//para estaticos sólo Model	//agregué archivo de carpeta y el modelo casaBrujas
	...
	Model miCubo("resources/objects/Cubito/miCubito.obj");
	Model cuboTnt("resources/objects/CuboText/tnt.obj");


	ModelAnim dummy("resources/objects/Dummy/HipHopDancing.dae");	//agrego muñeco de prueba
	dummy.initShaders(animShader.ID);
	// ANIMACIONES PROYECTO
	//Model arlo("resources/objects/Arlo/arlo.obj");
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		//animacionPersonaje.Draw(animShader); //animación comentada desde linea 280 aprox (ModelAnim)

		model = glm::translate(glm::mat4(1.0f), glm::vec3(100.0f, 0.0f, -20.0f)); // agrego al dummy
		model = glm::scale(model, glm::vec3(0.3f));	//Como es muy grande, bajo la escala
		animShader.setMat4("model", model);
		dummy.Draw(animShader);
		...
		model = glm::rotate(model, glm::radians(90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		animShader.setMat4("model", model);
		ninja.Draw(animShader); //animación comentada desde
		...
		// -------------------------------------------------------------------------------------------------------------------------
	/*
		model = glm::translate(glm::mat4(1.0f), glm::vec3(250.0f, 0.0f, -80.0f));
		model = glm::rotate(model, glm::radians(0.0f), glm::vec3(0.0f, 0.0f, 0.0f));
		staticShader.setMat4("model", model);
		arlo.Draw(staticShader);*/
		...
		//comienzo a dibujar casa
		model = glm::translate(glm::mat4(1.0f), glm::vec3(-120.0f, 0.0f, 30.0f));
		model = glm::scale(model, glm::vec3(3.0f));
		staticShader.setMat4("model", model);	//info se la paso al sahder
		casaBrujas.Draw(staticShader);
		//model = glm::rotate(model, glm::radians(-90.0f), glm::vec3(0.0f, 1.0f, 0.0f));
		
		model = glm::translate(glm::mat4(1.0f), glm::vec3(80.0f, 0.0f,40.0f));	//dibujo miCubito
		//model = glm::scale(model, glm::vec3(3.0f));
		staticShader.setMat4("model", model);	//info se la paso al shader
		miCubo.Draw(staticShader);

		model = glm::translate(glm::mat4(1.0f), glm::vec3(20.0f, 20.0f, 60.0f));	//dibujo cuboTnt
		//model = glm::scale(model, glm::vec3(3.0f));
		staticShader.setMat4("model", model);	//info se la paso al shader
		cuboTnt.Draw(staticShader);
		...
		model = glm::rotate(model, glm::radians(rot200), glm::vec3(0.0f, 1.0f, 0.0f));	//rotación para mover 180 el auto al llegar a la meta
